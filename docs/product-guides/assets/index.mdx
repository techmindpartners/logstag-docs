---
title: Assets
description: How Logstag's Assets area registers database servers, manages agent setup and connection state, and maps instances to external workflows.
---
...
Metric definitions for the Assets list and detail pages live in the [Overview reference](./overview.mdx)
and the [Detail reference](./detail.mdx). The Add a Database Server wizard is documented in the
[New Asset reference](./new.mdx).

## Access Model

The Assets area requires the **ManageDatabases** permission. Admin, SuperAdmin, and DBA hold it by
default; other roles are kept off `/assets` and its subroutes before the page loads.
