---
title: Audit Logs
description: How Logstag's Audit Logs page tracks API access and user activity, and why it's gated by the ViewAuditLogs permission.
---

# Audit Logs

Audit Logs provide a record of activity inside Logstag, gated by the **ViewAuditLogs** permission. The page is designed for security review, operational traceability, and administrative accountability.

Metric definitions for the Activity table and the Event Detail panel live in the
[Overview reference](./overview.mdx).

Audit Logs are not a general operator page. They require the ViewAuditLogs permission because they can expose user activity, administrative actions, access context, and sensitive operational metadata about how the organization uses Logstag.
...
Audit Logs are available to users whose role carries the **ViewAuditLogs** permission. Admin and SuperAdmin hold it by default; the audience is the permission itself, not a hardcoded role check, so a future role or a custom grant could carry it too.
...
Access is restricted by the ViewAuditLogs permission because Audit Logs can include:
...
Users with the ViewAuditLogs permission can enable additional columns when a deeper review is needed.
...
The column selector lets users with the ViewAuditLogs permission choose which audit fields are visible.
...
This helps distinguish direct user activity from authorized support or administrative access. Impersonation visibility matters for anyone reviewing with the ViewAuditLogs permission, for regulated environments, and for post-incident analysis.
...
A typical review flow for a user with the ViewAuditLogs permission is:
...
Because Audit Logs are available only to users with the ViewAuditLogs permission, they should not be used as the primary collaboration surface for incident response. When an operational issue needs team follow-up, a user with that permission can correlate audit evidence with Alerts, Assets, Integrations, monitoring reports, or explorer pages and share only the necessary operational context through the appropriate workflow.
